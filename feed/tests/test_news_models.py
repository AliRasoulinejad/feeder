...
class TestNewsUrls(AbstractNewsTest):
    def test_read_news(self):
        ...
        self.assertEqual(
            self.sample_user1.read_news.filter(news_id=self.sample_news.id).count(), 1
        )
        self.assertEqual(
            self.sample_user2.read_news.filter(news_id=self.sample_news.id).count(), 0
        )

    def test_bookmark_news(self):
        self.sample_news.bookmark_by_user_id(self.sample_user1.id)
        self.assertEqual(
            self.sample_user1.bookmarked_news.filter(
                news_id=self.sample_news.id
            ).count(),
            1,
        )
        self.assertEqual(
            self.sample_user2.bookmarked_news.filter(
                news_id=self.sample_news.id
            ).count(),
            0,
        )
    ...
    def test_favorite_news(self):
        ...
        self.assertEqual(
            self.sample_user1.favorites_news.filter(
                news_id=self.sample_news.id
            ).count(),
            1,
        )
        self.assertEqual(
            self.sample_user2.favorites_news.filter(
                news_id=self.sample_news.id
            ).count(),
            0,
        )
